...
class Window:
    ...
    def drawPoints(self):
        ...
        for s in grid_segments:
            ...
            self.drawnsegments.append(self.paper.create_line(
                p[0], p[1], q[0], q[1], fill="light gray"))
        ...
        for B in self.boxes:
            ...
            self.drawnsegments.append(self.paper.create_line(
                p1[0], p1[1], q1[0], p1[1], fill="black"))
            self.drawnsegments.append(self.paper.create_line(
                q1[0], p1[1], q1[0], q1[1], fill="black"))
            self.drawnsegments.append(self.paper.create_line(
                q1[0], q1[1], p1[0], q1[1], fill="black"))
            self.drawnsegments.append(self.paper.create_line(
                p1[0], q1[1], p1[0], p1[1], fill="black"))
        ...
        for p in self.points:
            ...
            if p.color == -1:
                self.drawnpoints.append(self.paper.create_oval(
                    x, y, x+2*self.t, y+2*self.t, outline="red", fill="red"))
            elif p.color == 1:
                self.drawnpoints.append(self.paper.create_oval(
                    x, y, x+2*self.t, y+2*self.t, outline="blue", fill="blue"))
            elif p.color == 2:
                self.drawnpoints.append(self.paper.create_oval(
                    x, y, x+2*self.t, y+2*self.t, outline="green", fill="green"))
        self.root.update()
    ...
    def leftclick(self, event):
        global start
        start = [int(self.paper.canvasx(event.x)),
                 int(self.paper.canvasy(event.y))]
    ...
    def release(self, event):
        global end
        end = [int(self.paper.canvasx(event.x)),
               int(self.paper.canvasy(event.y))]
        v = [-(end[0]-start[0]), (end[1]-start[1])]
        self.moveCenter([v[0], v[1]])
    ...
    def take_picture(self, filename=None):
        if filename == None:
            filename = asksaveasfilename(
                filetypes=[("postscript", "*.ps *.eps")], defaultextension=".ps")
